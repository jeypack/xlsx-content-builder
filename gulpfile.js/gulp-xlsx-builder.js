...
const buildNunjucks = () => {
  ...
  console.log("buildNunjucks", "pageName:", pageName, "templateFolder:", templateFolder);
  return (
    src(config.SRC_PATH + "pages/" + pageName + ".njk")
      .pipe(
        data(function (file) {
          ...
          const filename = config.SRC_PATH + "xlsx/" + templateFolder + xlsxName + ".xlsx";
...
const watchDirectory = (cb) => {
  ...
  watch(config.SRC_PATH + templateFolder + templateName + "assets/**/*", buildTemplateDev);
  watch(config.SRC_PATH + templateFolder + templateName + "*.scss", buildTemplateDev);
  ...
  watch(config.SRC_PATH + "templates/**/*.+(html|njk|nunjucks)", buildTemplateDev);
  cb();
};
...
const makeImages = (cb) => {
  ...
  const stream = src([path + '**/02_jpg/*.*']).pipe(
    data(function (file) {
      const parts = file.path.split('/').slice(-6);
      ...
      //const index = isNaN(Number(part)) ? 1 : Number(part);
      const obj = {
        index: Number(index),
...
const moveDefaultScss = (cb) => {
  ...
  return src([config.SRC_PATH + '**/*', '!' + config.SRC_PATH + 'scss/**', '!' + config.SRC_PATH + '_images/**', '!' + config.SRC_PATH + 'js/**', '!' + config.SRC_PATH + 'pages/**', '!' + config.SRC_PATH + 'templates/**', '!' + config.SRC_PATH + 'xlsx/**'])
    .pipe(
      data(function (file) {
...
const buildStream = (cb) => {
  ...
  while (counter < 4) {
    ...
    stream.add(src(config.SRC_PATH + 'config.json').pipe(
      data(function (file) {
        /* for (let key in file) {
        ...
        setDestination(noobFn);
        console.log("");
        console.log("buildStream", "data", "config destination:", config.destination);
        cleanDirectory(noobFn);
        return json;
      })
    ));
    ...
    stream.add(buildNunjucks(cb).pipe(
      data(function (file) {
